...
/// Based on: https://blogs.msdn.microsoft.com/wushuai/2018/03/10/extend-microsoft-aspnetcore-authentication-oauth-for-reverse-proxy/
type MyOAuthHandler (options: IOptionsMonitor<OAuthOptions>, logger: ILoggerFactory, encoder: UrlEncoder, clock: ISystemClock) =
  ...
  override __.HandleChallengeAsync (properties) =
    ...
    let authorizationEndpoint = __.BuildChallengeUrl (properties, __.BuildRedirectUri (__.Options.CallbackPath.Value)) // FIXME: Don't call .Value
    let redirectContext =
  ...
  member __.BuildRedirectUri (targetPath: string) =
    let scheme =
